...
export function AdminSidebar() {
  ...
  const items = [
    ...
    { name: "Lessons", href: "/admin", icon: BookOpen },
    // Future: categories, users, settings
    // { name: "Settings", href: "/admin/settings", icon: Settings },
  ];
  return (
    <aside className="w-60 shrink-0 h-screen sticky top-0 border-r bg-background">
      <div className="h-16 flex items-center px-4 border-b text-xl font-bold">Isyara Admin</div>
          ...
          return (
            <Link key={it.name} href={it.href} className="block">
              <Button
                ...
                className={cn("w-full justify-start", active && "bg-primary text-white")}
              >
